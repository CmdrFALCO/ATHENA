 ...
 *
 * Visual representation of the 7 places and 6 transitions in the
 * validation workflow. Places are rounded rectangles with token counts,
 * transitions are thin bars showing enabled state.
 */
...
const COL_T_3 = 800;         // T_commit / T_regenerate
const COL_PLACE_4 = 960;     // P_committed

const ROW_MAIN = 100;
const ROW_FEEDBACK = 260;
const ROW_REJECT = 380;
...
const EDGES: Edge[] = [
  // Main flow: P_proposals → T_validate → P_deciding → T_accept → P_verified → T_commit → P_committed
...
export function WorkflowGraph() {
  ...
  const enabledTransitions = useMemo(() => {
    ...
    if (has(PLACE_IDS.P_verified)) enabled.add(TRANSITION_IDS.T_commit);
