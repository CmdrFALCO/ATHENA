 ...
 *
 * Coordinates three strategies to gather relevant context:
 * 1. Selected Nodes - Explicit user-selected context (highest priority)
 * 2. Similarity Search - Semantically similar content to the query
 * 3. Graph Traversal - Connected nodes (1-hop expansion)
 */
...
export class ContextBuilder {
  ...
  async build(options: ContextOptions): Promise<ContextResult> {
    ...
    // Strategy 2: Similarity search (if query provided and room remains)
    ...
    // Strategy 3: Traversal (if enabled and room remains)
